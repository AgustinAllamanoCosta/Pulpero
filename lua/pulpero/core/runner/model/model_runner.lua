...
function Runner.run_local_model(self, tmp_prompt, config)
    self.logger:debug("Configuration ", config)

    local response_file = os.tmpname()

    local command = string.format(
        '%s -m %s --temp %s --ctx-size %s --threads %s --top_p %s --repeat-penalty 1.15'
        .. ' --repeat-last-n 32 --mirostat %s --mirostat-lr 0.05 --mirostat-ent 3.0'
