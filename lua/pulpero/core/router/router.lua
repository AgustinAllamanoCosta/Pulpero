...
function Router:detect_intention(user_message)
    ...
    local complete_prompt = string.format(prompts.intent_prompt, chat_history, user_message)
...
function Router:file_pipeline(user_message, file_context_data_str)
    local chat_history = self.history:generate_chat_history()
    local tool_description = self.tool_manager:generate_tools_description()
    ...
    local model_response_with_tool_call = self.model_runner:talk_with_model(prompt_file)
    ...
    local final_response = self:generate_final_response(user_message, tool_response)
    os.remove(prompt_file)
    return final_response
end

function Router:code_analysis_pipeline(user_message, file_context_data_str)
    local chat_history = self.history:generate_chat_history()
    local tool_description = self.tool_manager:generate_tools_description()
    local full_prompt = string.format(
        prompts.code,
...
function Router:general_chat_pipeline(user_message, file_context_data_str)
    local chat_history = self.history:generate_chat_history()
    ...
    local final_response = self.model_runner:talk_with_model(prompt_file)
